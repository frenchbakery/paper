...
In an IP network, every participating communication party (so called "host",such as a computer) is assigned an IP Address. The IP address is the lowest level of addressing most user-level applications have to deal with. It is a 32-Bit number typically expressed by four octets (bytes) in dot notation: \textbf{192.168.1.1}

The Internet Protocol provides the foundations for transmitting data between hosts on a network (or possibly multiple networks) in terms of IP network packets. These contain among other details, the sender and receiver addressing information and a block of data to be transmitted between the hosts. The operating system's network stack and other networking hardware then switch and route the packet from the sender to the desired destination host.
...
\begin{thebibliography}{00}
    ...
    \bibitem{b3} Websocket standart, URL:  https://www.rfc-editor.org/rfc/rfc6455.html
    \bibitem{b4} Information for WebSocket++, URL: https://github.com/zaphoyd/websocketpp/
